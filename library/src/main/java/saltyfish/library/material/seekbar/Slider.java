package saltyfish.library.material.seekbar;
...
import saltyfish.library.material.CustomView;
...
import static saltyfish.library.utils.SaltyfishUtilTool.getRelativeLeft;
import static saltyfish.library.utils.SaltyfishUtilTool.getRelativeTop;
import static saltyfish.library.utils.SaltyfishUtilTool.getSaltyfishInstance;


public class Slider extends CustomView implements ISaltyfishGeneralSkin {
	...
	private int     value               = 0;
	private int mLineColor=Color.parseColor("#B0B0B0");
	...
	public void invalidate() {
		ball.invalidate();
		if(isGeneralSkin){
			ball.setBackgroundDrawable(getSaltyfishInstance(SaltyfishSkinColorTool.class).changeDrawableColor(ball.getBackground()));
		}
		super.invalidate();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (value == min) {
			...
			paint.setStrokeWidth(getDpToPx(getContext(),2));
			...
			temp.drawCircle(ViewCompat.getX(ball) + ball.getWidth() / 2,
					ViewCompat.getY(ball) + ball.getHeight() / 2,
		...
		if (press && !showNumberIndicator) {
			...
			canvas.drawCircle(ViewCompat.getX(ball) + ball.getWidth() / 2,
	...
	public void onSkinChange() {
		...
		if (!isGeneralSkin)
			return;
		backgroundColor=getSaltyfishInstance(SaltyfishSkinColorTool.class).getPressedColor();
		ball.setBackgroundDrawable(getSaltyfishInstance(SaltyfishSkinColorTool.class).changeDrawableColor(ball.getBackground()));
		mLineColor=getSaltyfishInstance(SaltyfishSkinColorTool.class).getSkinColor();
		invalidate();
	}
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		// Set size of view
		setMinimumHeight(getDpToPx(getContext(),48));
		setMinimumWidth(getDpToPx(getContext(),80));
		...
		RelativeLayout.LayoutParams params = new LayoutParams((getDpToPx(getContext(),20
				)),getDpToPx(getContext(),20));
	...
	private void placeBall() {
		ViewCompat.setX(ball, getHeight() / 2 - ball.getWidth() / 2);
		ball.xIni = ViewCompat.getX(ball);
	...
	class Indicator extends RelativeLayout {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (animate) {
				...
				y -=getDpToPx(getContext(),6);
				size += getDpToPx(getContext(),2);
			}
			canvas.drawCircle(
					ViewCompat.getX(ball)
				...
				ViewCompat
						.setX(numberIndicator.numberIndicator,
								(ViewCompat.getX(ball)
										...
										- size);
				ViewCompat.setY(numberIndicator.numberIndicator, y - size);
