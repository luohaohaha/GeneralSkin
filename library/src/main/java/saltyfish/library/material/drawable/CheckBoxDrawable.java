package saltyfish.library.material.drawable;
...
import static saltyfish.library.utils.SaltyfishUtilTool.hasState;
...
public class CheckBoxDrawable extends Drawable implements Animatable {
	...
	private CheckBoxDrawable(int width, int height, int boxSize, int cornerRadius, int strokeSize,
							 ColorStateList strokeColor, int tickColor, int animDuration) {
		mWidth = width;
		mHeight = height;
		mBoxSize = boxSize;
	...
	protected void onBoundsChange(Rect bounds) {
		mBoxRect.set(bounds.exactCenterX() - mBoxSize / 2, bounds.exactCenterY() - mBoxSize / 2, bounds.exactCenterX()
				+ mBoxSize / 2, bounds.exactCenterY() + mBoxSize / 2);
	}
	...
	private void drawChecked(Canvas canvas) {
		...
		if (isRunning()) {
			if (mAnimProgress < FILL_TIME) {
				...
				mPaint.setColor(getMiddleColor(mPrevColor, mCurColor, progress));
				mPaint.setStrokeWidth(fillWidth);
				mPaint.setStyle(Paint.Style.STROKE);
				canvas.drawRect(mBoxRect.left + padding, mBoxRect.top + padding, mBoxRect.right - padding,
						mBoxRect.bottom - padding, mPaint);
	...
	protected boolean onStateChange(int[] state) {
		boolean checked = hasState(state, android.R.attr.state_checked);
	...
	public static class Builder {
		...
		public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
			TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CheckBoxDrawable, defStyleAttr,
					defStyleRes);

			width(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_cbd_width,
					getDpToPx(context, 32)));
			height(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_cbd_height,
					getDpToPx(context, 32)));
			boxSize(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_cbd_boxSize,
					getDpToPx(context, 18)));
			cornerRadius(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_cbd_cornerRadius,
					getDpToPx(context, 2)));
			strokeSize(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_cbd_strokeSize,
					getDpToPx(context, 2)));
			...
			animDuration(a.getInt(R.styleable.CheckBoxDrawable_cbd_animDuration,
			...
			if (mStrokeColor == null) {
				...
				int[] colors = new int[] { colorControlNormal(context, 0xFF000000),
		...
		public CheckBoxDrawable build() {
			...
			return new CheckBoxDrawable(mWidth, mHeight, mBoxSize, mCornerRadius, mStrokeSize, mStrokeColor,
					mTickColor, mAnimDuration);
		}
