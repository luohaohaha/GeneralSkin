...
import static saltyfish.library.utils.SaltyfishSizeFormatter.getDpToPx;
import static saltyfish.library.utils.SaltyfishUtilTool.FRAME_DURATION;
import static saltyfish.library.utils.SaltyfishUtilTool.colorControlHighlight;
import static saltyfish.library.utils.SaltyfishUtilTool.getColor;
import static saltyfish.library.utils.SaltyfishUtilTool.getType;
...
public class RippleDrawable extends Drawable implements Animatable, OnTouchListener {
	...
	private RippleDrawable(Drawable backgroundDrawable, int backgroundAnimDuration, int backgroundColor,
						   int rippleType, int delayClickType, int maxRippleRadius, int rippleAnimDuration, int rippleColor,
						   Interpolator inInterpolator, Interpolator outInterpolator, int type, int topLeftCornerRadius,
						   int topRightCornerRadius, int bottomRightCornerRadius, int bottomLeftCornerRadius, int left, int top,
		...
		setMask(type, topLeftCornerRadius, topRightCornerRadius, bottomRightCornerRadius, bottomLeftCornerRadius, left,
				top, right, bottom);
		...
		mInShader = new RadialGradient(0, 0, GRADIENT_RADIUS, new int[] { mRippleColor, mRippleColor, 0 },
				GRADIENT_STOPS, Shader.TileMode.CLAMP);
		if (mRippleType == TYPE_WAVE)
			mOutShader = new RadialGradient(0, 0, GRADIENT_RADIUS, new int[] { 0,
					getColor(mRippleColor, 0f), mRippleColor }, GRADIENT_STOPS, Shader.TileMode.CLAMP);
	}
	...
	public void setMask(int type, int topLeftCornerRadius, int topRightCornerRadius, int bottomRightCornerRadius,
	...
	protected void onBoundsChange(Rect bounds) {
		...
		mBackgroundBounds.set(bounds.left + mMask.left, bounds.top + mMask.top, bounds.right - mMask.right,
				bounds.bottom - mMask.bottom);
		mBackground.reset();
	...
	private void updateTouch() {
		if (mState != STATE_RELEASE) {
			float backgroundProgress = Math.min(1f, (float) (SystemClock.uptimeMillis() - mStartTime)
					/ mBackgroundAnimDuration);
			mBackgroundAlphaPercent = mInInterpolator.getInterpolation(backgroundProgress)
			...
			float backgroundProgress = Math.min(1f, (float) (SystemClock.uptimeMillis() - mStartTime)
					/ mBackgroundAnimDuration);
			mBackgroundAlphaPercent = (1f - mOutInterpolator.getInterpolation(backgroundProgress))
					* Color.alpha(mBackgroundColor) / 255f;
	...
	public static class Mask {
		...
		public Mask(int type, int topLeftCornerRadius, int topRightCornerRadius, int bottomRightCornerRadius,
				int bottomLeftCornerRadius, int left, int top, int right, int bottom) {
			this.type = type;
	...
	public static class Builder {
		...
		public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
			...
			backgroundAnimDuration(a.getInteger(R.styleable.RippleDrawable_rd_backgroundAnimDuration, context
			...
			if (type >= TypedValue.TYPE_FIRST_INT && type <= TypedValue.TYPE_LAST_INT)
				...
				maxRippleRadius(a.getDimensionPixelSize(R.styleable.RippleDrawable_rd_maxRippleRadius,
						getDpToPx(context, 48)));
			rippleColor(a.getColor(R.styleable.RippleDrawable_rd_mrippleColor,
					colorControlHighlight(context, 0)));
			rippleAnimDuration(a.getInteger(R.styleable.RippleDrawable_rd_rippleAnimDuration, context.getResources()
			...
			topRightCornerRadius(a.getDimensionPixelSize(R.styleable.RippleDrawable_rd_topRightCornerRadius,
					mMaskTopRightCornerRadius));
			bottomRightCornerRadius(a.getDimensionPixelSize(R.styleable.RippleDrawable_rd_bottomRightCornerRadius,
					mMaskBottomRightCornerRadius));
			bottomLeftCornerRadius(a.getDimensionPixelSize(R.styleable.RippleDrawable_rd_bottomLeftCornerRadius,
		...
		public RippleDrawable build() {
			...
			return new RippleDrawable(mBackgroundDrawable, mBackgroundAnimDuration, mBackgroundColor, mRippleType,
					mDelayClickType, mMaxRippleRadius, mRippleAnimDuration, mRippleColor, mInInterpolator,
					mOutInterpolator, mMaskType, mMaskTopLeftCornerRadius, mMaskTopRightCornerRadius,
					mMaskBottomRightCornerRadius, mMaskBottomLeftCornerRadius, mMaskLeft, mMaskTop, mMaskRight,
					mMaskBottom);
		}
