package saltyfish.library.material.cswitch;
...
import saltyfish.library.material.RippleDrawable;
import saltyfish.library.material.RippleManager;
...
import static saltyfish.library.utils.SaltyfishUtilTool.colorControlActivated;
import static saltyfish.library.utils.SaltyfishUtilTool.colorControlNormal;
import static saltyfish.library.utils.SaltyfishUtilTool.getColor;
import static saltyfish.library.utils.SaltyfishUtilTool.getMiddleColor;
import static saltyfish.library.utils.SaltyfishUtilTool.getSaltyfishInstance;


public class Switch extends View implements Checkable ,ISaltyfishGeneralSkin {
	...
	public void onSkinChange() {
		...
		if (!isGeneralSkin)
			return;
		mTrackColors = getSaltyfishInstance(SaltyfishSkinColorTool.class).getSwitchColorStateList();
		mThumbColors = getSaltyfishInstance(SaltyfishSkinColorTool.class).getSwitchColorStateList();
	...
	private void applyStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		mTrackSize = a.getDimensionPixelSize(R.styleable.Switch_sw_trackSize, getDpToPx(context, 2));
		mTrackColors = isGeneralSkin?getSaltyfishInstance(SaltyfishSkinColorTool.class).getSwitchColorStateList():a.getColorStateList(R.styleable.Switch_sw_trackColor);
		...
		mThumbColors = isGeneralSkin?getSaltyfishInstance(SaltyfishSkinColorTool.class).getSwitchColorStateList():a.getColorStateList(R.styleable.Switch_sw_thumbColor);
		mThumbRadius = a.getDimensionPixelSize(R.styleable.Switch_sw_thumbRadius,
				getDpToPx(context, 8));
		mShadowSize = a.getDimensionPixelSize(R.styleable.Switch_sw_thumbElevation,
				getDpToPx(context, 2));
		mShadowOffset = mShadowSize / 2;
		mMaxAnimDuration = a.getInt(R.styleable.Switch_sw_animDuration,
		...
		if (mThumbColors == null) {
			int[][] states = new int[][] { new int[] { -android.R.attr.state_checked },
					new int[] { android.R.attr.state_checked }, };
			int[] colors = new int[] { 0xFAFAFA, colorControlActivated(context, 0xFF000000), };
	 ...
	 * 
	 * @param listener
	 *            The {@link OnCheckedChangeListener} will be called.
	 */
	...
	/**
	 * Change the checked state of this Switch immediately without showing
	 * animation.
	 * 
	 * @param checked
	 *            The checked state.
	...
	public int getSuggestedMinimumHeight() {
		return mThumbRadius * 2 + Math.max(mShadowSize - mShadowOffset, getPaddingTop())
				+ Math.max(mShadowSize + mShadowOffset, getPaddingBottom());
	}
	...
	private void buildShadow() {
		...
		mShadowPaint.setShader(new RadialGradient(0, 0, mThumbRadius + mShadowSize, new int[] { COLOR_SHADOW_START,
				COLOR_SHADOW_START, COLOR_SHADOW_END }, new float[] { 0f, startRatio, 1f }, Shader.TileMode.CLAMP));
		...
		if (mTrackCap != Paint.Cap.ROUND) {
			...
			if (x - radius > mDrawRect.left) {
				float angle2 = (float) (Math
						.acos(Math.max(0f, (mDrawRect.left + halfStroke - x + radius) / halfStroke)) / Math.PI * 180);
			...
			if (x + radius < mDrawRect.right) {
				float angle2 = (float) Math
						.acos(Math.max(0f, (x + radius - mDrawRect.right + halfStroke) / halfStroke));
				mTrackPath.moveTo((float) (mDrawRect.right - halfStroke + Math.cos(angle2) * halfStroke),
