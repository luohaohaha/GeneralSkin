...
/**
 * ClassName: SaltyfishLinearColorPicker<br/>
 ...
 * <p/>
 */
public class SaltyfishLinearColorPicker extends View {

	public SaltyfishLinearColorPicker(Context context) {
		super(context);
		initAttrs(context, null);
	}

	public SaltyfishLinearColorPicker(Context context, AttributeSet attrs) {
		super(context, attrs);
		initAttrs(context, attrs);
	}

	public SaltyfishLinearColorPicker(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		...
		// 滑块
		mThumb = a.getDrawable(R.styleable.Saltyfish_Custom_thumbDrawable);
		// 方向
		mOrientation = a.getInt(R.styleable.Saltyfish_Custom_color_orientation, HORIZONTAL);
		// 颜色条宽度
		mColorPanelWidth = a.getDimensionPixelOffset(R.styleable.Saltyfish_Custom_colorPanelWidth, mDefaultHeight);
		// 颜色数组
		int colorArrayId = a.getResourceId(R.styleable.Saltyfish_Custom_gradientArray, -1);
		if (-1 != colorArrayId) {
	...
	protected void onDraw(Canvas canvas) {
		...
		//回调当前颜色
		measureProgress();
		currentColor = getCurrentColor(isHorizontal() ? lastX : lastY);
		if (null != mSelectListener) {
			mSelectListener.onColorSelect(currentColor, progress);
		}
	}
